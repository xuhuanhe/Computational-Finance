...
@Student Name  : Xuhuan He, Lin Shi
...
class BinomialModel(object):
    ...
    def _binomial_american_put(self, S_0, K, T, r, sigma, q, N):
        ...
        for k in range(N-1, -1, -1):
            for m in range(0,k+1):
                early_exercise_value = K - S_0 * (u ** (m)) * (d ** (k-m))
                european_call_value = math.exp(-r * dt) * (prob * P[(k+1, m+1)] + (1-prob) * P[(k+1, m)])
                P[(k, m)] = max(european_call_value, early_exercise_value)
        # end TODO
        return P[(0,0)]        
